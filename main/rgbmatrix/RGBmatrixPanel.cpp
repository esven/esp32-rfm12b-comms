...
// be specified as any pin within a specific PORT register stated below.
 ...
 #define DATADIR  DDRD
 #define CLKPORT  PORTB
#elif defined(ARDUINO_ARCH_SAMD) || defined(__xtensa__)
...
#if defined(ARDUINO_ARCH_SAMD) || defined(__xtensa__)
  ,uint8_t *pinlist
#endif
  ) {
#if defined(ARDUINO_ARCH_SAMD) || defined(__xtensa__)
...
static hw_timer_reg_t *hw_timer = (hw_timer_reg_t *)(DR_REG_TIMERGROUP0_BASE);
#endif
...
void RGBmatrixPanel::begin(void) {
  ...
  if (_clk < 32) {
    ...
    TIMERG0.int_ena.val &= ~BIT(0);
    hw_timer->config.enable = 0;
    hw_timer->config.divider = 0x10;
    hw_timer->config.increase = true;
    hw_timer->config.autoreload = true;
    static intr_handle_t intr_handle = NULL;
    if(intr_handle){
    	esp_intr_disable(intr_handle);
    }
    hw_timer->config.edge_int_en = 1;
    hw_timer->config.level_int_en = 0;
    int intr_source = ETS_TG0_T0_EDGE_INTR_SOURCE + 0;
    esp_intr_alloc(intr_source, (int)(ESP_INTR_FLAG_IRAM|ESP_INTR_FLAG_LOWMED|ESP_INTR_FLAG_EDGE), IRQ_HANDLER, NULL, &intr_handle);
    TIMERG0.int_ena.val |= BIT(0);
    esp_intr_enable(intr_handle);
    hw_timer->load_high = 0;
    hw_timer->load_low = 0;
    hw_timer->reload = 1;
    hw_timer->config.enable = 1;
// TODO: Enable Timer
#endif
